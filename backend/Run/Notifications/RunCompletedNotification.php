...
class RunCompletedNotification extends NotificationBase implements ShouldQueue
{
    use Queueable;
    ...
    private function getTitle(): string
    {
        if ($this->run->contract) {
            return "Build completed for {$this->run->contract->name}";
        }
    ...
    {
        if ($this->run->contract) {
            return 'You can now deploy it from the dashboard.';
        }
